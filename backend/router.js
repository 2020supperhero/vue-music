...
// 获取一个随机数值
const getRandomVal = function(prefix = '') {
...
// 注册推荐列表接口路由
const registerRecommend = function(app) {
  app.get('/api/getRecommend', (req, res) => {
    ...
    // const sign = 'zzb4b5adf4emghrndbrslfbaaohgt85qd8c2607f'
    // 发送 get 请求
    get(url, {
      sign,
      '-': randomVal,
...
// 注册歌手列表接口路由
const registerSingerList = function(app) {
...
// 注册歌手详情接口路由
const registerSingerDetail = function(app) {
...
// 处理歌曲列表
const handleSongList = function(list) {
  const songList = []
  list.forEach(item => {
...
// 合并多个歌手的姓名
const mergeSinger = function(singer) {
  const ret = []
  if (!singer) {
    return ''
  }
...
const registerSongsUrl = function(app) {
...
// 注册歌词接口
const registerlyric = function(app) {
...
// 注册热门搜索接口
const registerHotKeys = function(app) {
...
// 注册搜索查询接口
const registerSearch = function(app) {
  app.get('/api/search', (req, res) => {
    ...
    console.log('query, page, showSinger-->', query, page, showSinger)
